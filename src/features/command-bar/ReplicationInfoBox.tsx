...
const ReplicationInfoBox = ({ info, onSave, onClose }: Props) => {
  const [reps, setReps] = useState(String(info.count));

  // Recalcular as repetições não remonta a caixa: sem isto o campo ficaria com
  // o número obsoleto enquanto o resto da UI já mostra o novo máximo.
  useEffect(() => setReps(String(info.count)), [info.count]);

  const parsed = parseInt(reps, 10);
  const effective = Number.isNaN(parsed) ? 1 : Math.max(1, Math.min(parsed, info.count));
