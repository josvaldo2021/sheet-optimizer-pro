...
// ========== STRIP PACKING WITH DYNAMIC PROGRAMMING ==========

interface Strip {
  height: number;
  pieces: Array<{ piece: Piece; w: number; h: number; rotated: boolean }>;
  totalWidth: number;
  totalArea: number;
}

/**
 * Gera faixas horizontais agrupando peças com alturas compatíveis (dentro da tolerância).
 * Usa First-Fit-Decreasing (FFD) para empacotar peças em cada faixa.
 *
 * @param heightTolerance - tolerância em mm para agrupar alturas (ex: 5 = ±5mm)
 * @param forceRotation - 'natural' = menor dim como altura; 'rotated' = maior dim como altura
 */
function generateStrips(
  pieces: Piece[],
  usableW: number,
  heightTolerance: number = 5,
  forceRotation: 'natural' | 'rotated' = 'natural',
): { strips: Strip[]; unmapped: Piece[] } {
  type OP = { piece: Piece; w: number; h: number; rotated: boolean };

  const oriented: OP[] = pieces.map(p => {
    if (forceRotation === 'rotated') {
      // Larger dim as height (narrow strip height = large dim)
      if (p.w <= p.h) return { piece: p, w: p.w, h: p.h, rotated: false };
      return { piece: p, w: p.h, h: p.w, rotated: true };
    }
    // natural: smaller dim as height (wider strips)
    if (p.w >= p.h) return { piece: p, w: p.w, h: p.h, rotated: false };
    return { piece: p, w: p.h, h: p.w, rotated: true };
  });

  // Sort by height ascending for grouping
  oriented.sort((a, b) => a.h - b.h || b.w - a.w);

  // Group by compatible heights (within tolerance)
  const heightGroups: OP[][] = [];
  const used = new Array(oriented.length).fill(false);

  for (let i = 0; i < oriented.length; i++) {
    if (used[i]) continue;
    const group: OP[] = [oriented[i]];
    used[i] = true;
    const baseH = oriented[i].h;

    for (let j = i + 1; j < oriented.length; j++) {
      if (used[j]) continue;
      if (oriented[j].h - baseH <= heightTolerance) {
        group.push(oriented[j]);
        used[j] = true;
      }
    }
    heightGroups.push(group);
  }

  const strips: Strip[] = [];
  const unmapped: Piece[] = [];

  for (const group of heightGroups) {
    const sorted = [...group].sort((a, b) => b.w - a.w); // FFD
    const remaining = [...sorted];

    while (remaining.length > 0) {
      const strip: Strip = { height: 0, pieces: [], totalWidth: 0, totalArea: 0 };

      for (let i = 0; i < remaining.length; i++) {
        if (strip.totalWidth + remaining[i].w <= usableW) {
          strip.pieces.push(remaining[i]);
          strip.totalWidth += remaining[i].w;
          strip.height = Math.max(strip.height, remaining[i].h);
          strip.totalArea += remaining[i].w * remaining[i].h;
      ...
      if (strip.pieces.length === 0) {
        unmapped.push(remaining.shift()!.piece);
        continue;
      }
  ...
  return { strips, unmapped };
}

/**
 * Programação Dinâmica (Knapsack 0-1) para selecionar a melhor combinação
 * de faixas que maximize a área total dentro da altura disponível.
 */
function dpSelectStrips(strips: Strip[], usableH: number): number[] {
  const H = Math.floor(usableH);
  const n = strips.length;
  if (n === 0) return [];

  const dp = new Float64Array(H + 1);
  const take = new Uint8Array(n * (H + 1));

  for (let i = 0; i < n; i++) {
    const sh = Math.ceil(strips[i].height);
    const sa = strips[i].totalArea;
    if (sh > H) continue;
    const offset = i * (H + 1);

    // Backwards traversal: 0-1 knapsack
    for (let j = H; j >= sh; j--) {
      if (dp[j - sh] + sa > dp[j]) {
        dp[j] = dp[j - sh] + sa;
        take[offset + j] = 1;
  ...
  // Backtrack to find selected strips
  const selected: number[] = [];
  let rem = H;
  for (let i = n - 1; i >= 0; i--) {
    if (take[i * (H + 1) + rem]) {
      selected.push(i);
      rem -= Math.ceil(strips[i].height);
    }
  }
...
/**
 * Strip Packing completo: gera faixas, seleciona via DP, constrói árvore,
 * e preenche espaços residuais com peças restantes.
 */
function runStripPlacement(
  ...
  const { strips, unmapped } = generateStrips(pieces, usableW, heightTolerance, forceRotation);

  if (strips.length === 0) {
    return { tree: createRoot(usableW, usableH), area: 0, remaining: [...pieces] };
  }

  const selectedIndices = dpSelectStrips(strips, usableH);
  ...
  let totalArea = 0;
  const usedPieceSet = new Set<Piece>();

  // Sort selected strips by height descending (tallest bands first for better packing)
  const sortedSelected = [...selectedIndices].sort(
    (a, b) => strips[b].height - strips[a].height
  );

  // Single X column of full sheet width
  insertNode(tree, "root", "X", usableW, 1);
  const xNode = tree.filhos[0];

  for (const idx of sortedSelected) {
    const strip = strips[idx];
    const usedH = xNode.filhos.reduce((a, y) => a + y.valor * y.multi, 0);
    const freeH = usableH - usedH;

    if (strip.height > freeH + 0.5) continue;

    // Residual dominance: if remaining height can't fit more strips, extend this one
    let yHeight = strip.height;
    const residualH = freeH - strip.height;
    if (residualH > 0 && residualH < 50) {
      // Check if any remaining strip could fit in residualH
      const canFitMore = strips.some((s, si) =>
        !sortedSelected.includes(si) || si === idx
          ? false
          : Math.ceil(s.height) <= residualH
      );
      if (!canFitMore) yHeight = freeH;
    }

    const yId = insertNode(tree, xNode.id, "Y", yHeight, 1);
    const yNode = findNode(tree, yId)!;

    for (const sp of strip.pieces) {
      createPieceNodes(tree, yNode, sp.piece, sp.w, sp.h, sp.rotated);
      totalArea += sp.w * sp.h;
      usedPieceSet.add(sp.piece);
    }
  }

  // Collect remaining pieces (not selected by DP + unmapped)
  const remaining = pieces.filter(p => !usedPieceSet.has(p));
  remaining.push(...unmapped);

  // Fill voids with remaining pieces
  if (remaining.length > 0) {
    totalArea += fillVoids(tree, remaining, usableW, usableH, minBreak);
  }

  totalArea = clampTreeHeights(tree, usableW, usableH, totalArea);
