...
| `unique-per-sheet.ts` | Módulo **puro** da restrição "peça única por chapa" (spec 009): `splitMarked`, `perSheetQty`, `capForSheet` (linha marcada `uniquePerSheet` → ≤1 por chapa), `sheetInvKey` (chave de cache sobre a fatia capada), `countMarkedOnSheet` (contagem via árvore, `extractLeafPieces`). Enforçado no `runAllSheets` de `Index.tsx` (montagem do `inv` + chave do cache); motor/WASM intocados. | Para alterar como a marcação limita a repetição por chapa ou a chave de cache do plano. |
