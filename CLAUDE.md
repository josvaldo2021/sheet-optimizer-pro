...
`optimizeGeneticAsync` roda num **POOL de Web Workers** (`engine.worker.ts` + pool no `engine-adapter.ts`; worker único em 8892e79, pool em 1ec1060). O pool serve os 3-4 CANDIDATOS do plano, que rodam em `Promise.all` (~2x mais rápido; `Promise.all` preserva a ordem ⇒ `candidateGroups` segue determinístico). `optimizeV6`/`runPlacement` seguem SÍNCRONOS na thread principal (são callbacks injetados em `buildJumboSheet`/XFill) — é por isso que o ganho não chega ao nº de candidatos. Qualquer falha desliga o pool e degrada para a thread principal; em jsdom `typeof Worker === "undefined"`, então os testes usam esse caminho. `localStorage.enginePoolSize` sobrepõe o tamanho do pool (pool 1 = comportamento em série, sem precisar de outro build).  
...
7a. **INTERCALE as configurações ao comparar tempos** — a máquina do usuário tem 26-34% de CPU ocupada por outra coisa, e a carga VARIA. Medindo em blocos (4x config A, depois 4x config B), a MESMA configuração deu de 20 s a 86 s e as conclusões se contradisseram três vezes. Medindo A/B/A/B na mesma sessão, os números fecharam. Prefira um override em runtime (como `localStorage.enginePoolSize`) a rebuildar entre amostras. E reporte a SEPARAÇÃO entre os conjuntos, não só a média: "todas as corridas em série ≥ 51 s, todas as paralelas ≤ 46 s" é uma afirmação honesta com ruído alto; uma média não é.
